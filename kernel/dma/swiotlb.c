...
retry:
	...
	slots_order = get_order(array_size(sizeof(*mem->slots), nslabs));
	mem->slots = (void *)__get_free_pages(GFP_KERNEL | __GFP_ZERO, slots_order);
	...
	mem->areas = (struct io_tlb_area *)kcalloc(num_areas,
						   ...
						   GFP_KERNEL);
	if (!mem->areas) {
		free_pages((unsigned long)mem->slots, slots_order);
		return -ENOMEM;
	}
...
static int swiotlb_find_slots(struct device *dev, phys_addr_t orig_addr,
			      size_t alloc_size, unsigned long alloc_align_mask)
{
	struct io_tlb_mem *mem = dev->dma_io_tlb_mem;
